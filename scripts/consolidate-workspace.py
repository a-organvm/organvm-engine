...
def plan_migration() -> dict:
    ...
    for item in WORKSPACE.iterdir():
        if (
            (item.name.startswith("export-") and item.name.endswith(".md"))
            or (item.name.startswith("2026-") and item.suffix in (".txt", ".md"))
            or (item.name.startswith("session-") and item.name.endswith(".md"))
            or item.name in ("sync_interlinked_landing_pages.sh", "text-based--relevance.md")
        ):
